...
Порядок SDD-задач: **3.11.1** (токены) → **3.11.2** (компоненты); после 3.11.2 независимые **3.11.3** (авторизация), **3.11.4** (публичный MVP-лендинг) и **5.13.21** (переключатель абонементов организации), затем **8.10.1** (Mini App игрока), **8.10.2** (Mini App организатора), **5.15.1** (desktop организатора) → **6.10.1** (desktop-оплаты/касса) → **8.10.3** (сквозной QA, включая лендинг). Независимые задачи могут идти параллельно только в отдельных ветках и с отдельными gates. Лендинг сохраняет композицию `0 Лендинг.html`, но не публикует credits-тарифы, цены и обещания функций вне MVP.
