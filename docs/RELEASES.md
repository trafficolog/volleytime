...
Порядок SDD-задач: **3.11.1** (токены) → **3.11.2** (компоненты) → **3.11.3** (авторизация) → **3.11.6** (исправление выхода после OTP); затем **8.10.1** (Mini App игрока), **8.10.2** (Mini App организатора), **5.15.1** (desktop организатора) → **6.10.1** (desktop-оплаты/касса) → **8.10.3** (сквозной QA). Независимые задачи после 3.11.2 могут идти параллельно только в отдельных ветках и с отдельными gates.
